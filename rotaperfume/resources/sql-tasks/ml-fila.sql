...
-- ============================================================================
--
-- Estratégia: substituir o RTAS (que não suporta schema em UC quando o
-- FROM referencia outros schemas) por CREATE TABLE + INSERT.
...
FROM (
    ...
    FROM (
        SELECT
            ...
            ROW_NUMBER() OVER (PARTITION BY c.vendedor_id ORDER BY s.score DESC) AS ordem
...
SELECT CASE WHEN (SELECT COUNT(*) FROM lakehouse_rotaperfume.gold.fila_semanal
...
SELECT CASE WHEN NOT EXISTS (
         SELECT 1 FROM lakehouse_rotaperfume.gold.fila_semanal
